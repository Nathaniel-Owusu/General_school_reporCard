...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Helper
        function safe_json($val) { return json_encode($val ?? []); }
        ...
        // Insert Users
        if (!empty($data['users'])) {
            $stmt = $conn->prepare("INSERT INTO users (id, school_id, name, email, password, role, assigned_classes) VALUES (?, ?, ?, ?, ?, ?, ?)");
            foreach($data['users'] as $u) {
                $ref_classes = safe_json($u['assigned_classes'] ?? []);
                $stmt->bind_param("sssssss", $u['id'], $u['school_id'], $u['name'], $u['email'], $u['password'], $u['role'], $ref_classes);
                $stmt->execute();
            }
            $stmt->close();
        }
        ...
        if (!empty($data['classes'])) {
            $stmt = $conn->prepare("INSERT INTO classes (id, school_id, name, subjects) VALUES (?, ?, ?, ?)");
            foreach($data['classes'] as $c) {
                $ref_subs = safe_json($c['subjects'] ?? []);
                $stmt->bind_param("ssss", $c['id'], $c['school_id'], $c['name'], $ref_subs);
                $stmt->execute();
            }
            $stmt->close();
        }
